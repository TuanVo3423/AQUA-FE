            ...
            <div className="flex items-center justify-between flex-grow pl-12">
              ...
              {hasUser ? (
                <div className="flex gap-x-3">
                  <Link to={"/account"}>
                    <p className="text-white">{hasUser.name}</p>
                  </Link>
                  <a
                    href="login.html"
                    className="ml-auto underline justify-self-end text-gray-200 hover:text-white transition"
                  >
                    Logout
                  </a>
                </div>
                      ...
                      <React.Fragment key={item.id}>
                        <Link to={item.path}>
                          <p
                            onClick={() =>
                              dispatch(
                                NavbarSlice.actions.setIndexNavbarTitle(item.id)
                              )
                            }
                            className={`ml-auto justify-self-end font-semibold ${
                              location.pathname === item.path
                                ? "border-b-[2px] border-primary "
                                : ""
                            } uppercase text-gray-200 hover:text-primary transition`}
                          >
                            {item.title}
                          </p>
                        </Link>
